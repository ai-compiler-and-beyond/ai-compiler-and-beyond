...
#### Architecture of AI Accelerators
  ...
  - Google TPU: [Ten Lessons From Three Generations Shaped Google’s TPUv4i : Industrial Product](https://ieeexplore.ieee.org/abstract/document/9499913)**  — Norman P. Jouppi, David Patternson, et al., *ISCA 2021*. (~14 pages) <details close> 
    <summary><b>Supplementary Analyses and Insights</b></summary> 
      ...
      semiconductor technology advances unevenly;

      compiler compatibility matters more than binary compatibility, especially for VLIW domain-specific architectures (DSAs);

      the target total cost of ownership outweighs initial cost;

      support for multi-tenancy is essential;

      deep neural networks (DNNs) grow at a rate of 1.5× per year;

      the evolution of DNNs continuously shifts workloads;

      some inference tasks still require floating-point computation;

      inference DSAs require air cooling;

      applications are constrained by latency rather than batch size;

      backward ML compatibility enables rapid deployment of DNNs.
